...
const MVPSubmissionComponent = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.problemStatement.trim()) {
      ...
      if (wordCount < 10) {
        errors.problemStatement = 'Problem statement must be at least 10 words';
      } else if (wordCount > 150) {
        errors.problemStatement = 'Problem statement must not exceed 150 words';
      }
    }

    if (!formData.mvpFeatures.trim()) {
      errors.mvpFeatures = 'Please describe your MVP features';
    } else if (formData.mvpFeatures.trim().length < 50) {
      errors.mvpFeatures = 'Please provide a more detailed description (at least 50 characters)';
    }
  ...
  const handleSubmit = async () => {
    ...
    const submissionData = {
      ...
      mvpFeatures: formData.mvpFeatures.trim()
    };
    ...
    try {
      ...
      if (data.success) {
        console.log('✅ MVP features submitted successfully:', data);
        setMessageType('success');
        setMessageContent({
          title: 'MVP Features Submitted Successfully! 🎉',
          description: 'Your MVP features have been submitted successfully. Our team will review your submission and get back to you soon. Good luck!'
        });
        setShowMessage(true);
  ...
  const getWordCount = (field) => {
    const text = formData[field].trim();
  ...
  return (
    <div className="min-h-screen text-white p-4 sm:p-8">
      ...
      <div className={`text-center mb-8 transition-all duration-1000 ${
        ...
        <h1 className="text-4xl sm:text-5xl font-bold text-white mb-4">MVP Features Submission</h1>
        <p className="text-green-400/80 text-lg">Share your Minimum Viable Product features for Omnitrix Hackathon 2025</p>
      </div>
      ...
      <div className="max-w-4xl mx-auto mb-8">
        <div className="bg-transparent border-2 border-blue-400/60 rounded-xl p-6 shadow-lg">
          <div className="flex items-start space-x-4">
            ...
            <div>
              <h3 className="text-xl font-bold text-blue-400 mb-2">Important Guidelines</h3>
              <ul className="text-blue-200 space-y-2 text-sm">
                <li>• Use your registered Team ID and Team Name</li>
                <li>• Problem statement should be concise (10-150 words)</li>
                <li>• Clearly explain your top MVP features with implementation details</li>
                <li>• Focus on the core functionalities that make your solution unique</li>
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {domains.map((domain, index) => (
                  <option key={index} value={domain} className="bg-gray-800 text-white">
                    {domain}
                  </option>
                ))}
              </select>
              {validationErrors.domain && (
                <p className="text-red-400 text-sm mt-1">{validationErrors.domain}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <label className="block text-white font-semibold text-lg">
                  Problem Statement (1-2 lines) <span className="text-red-400">*</span>
                </label>
                <span className={`text-sm font-medium ${
                  getWordCount('problemStatement') > 150 ? 'text-red-400' : 
                  getWordCount('problemStatement') < 10 ? 'text-yellow-400' : 'text-green-400'
                }`}>
                  {getWordCount('problemStatement')}/150 words
                </span>
              </div>
              <textarea
              ...
              <p className="text-gray-400 text-xs">Keep it concise - maximum 150 words</p>
            </div>

            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <label className="block text-white font-semibold text-lg">
                  Explain Your Top MVP Features <span className="text-red-400">*</span>
                </label>
                <span className={`text-sm font-medium ${
                  getCharCount('mvpFeatures') < 50 ? 'text-yellow-400' : 'text-green-400'
                }`}>
                  {getCharCount('mvpFeatures')} characters
                </span>
              </div>
              <textarea
                name="mvpFeatures"
                ...
                rows={10}
                className={`w-full bg-gray-900/50 border-2 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none transition-all duration-300 resize-none ${
                  validationErrors.mvpFeatures 
                    ? 'border-red-400/60 focus:border-red-400' 
                    : 'border-green-400/30 focus:border-green-400/60'
                }`}
                placeholder="Describe your core MVP features in detail:&#10;&#10;1. Feature Name - Detailed explanation of what it does and how it works&#10;2. Feature Name - Technical implementation and user benefits&#10;3. Feature Name - Integration details and unique value proposition&#10;&#10;Include technical details, user benefits, and what makes each feature essential to your MVP..."
              />
              {validationErrors.mvpFeatures && (
                <p className="text-red-400 text-sm mt-1">{validationErrors.mvpFeatures}</p>
              )}
              <div className="bg-gray-900/30 border border-green-400/20 rounded-lg p-4">
                <p className="text-gray-300 text-sm mb-2 font-medium">What to include:</p>
                <ul className="text-gray-400 text-xs space-y-1 list-disc list-inside">
                  <li>Core features that define your MVP</li>
                  <li>Technical implementation details</li>
                  <li>User benefits and value proposition</li>
                  <li>How features integrate with your tech stack</li>
                  <li>What makes your solution unique or innovative</li>
                </ul>
              </div>
            </div>
            ...
            <div className="pt-6">
              ...
              >
                {isLoading ? 'Submitting...' : 'Submit MVP Features'}
              </button>
              
              {!isFormValid() && !isLoading && (
                <p className="text-yellow-400 text-sm mt-2 text-center">
                  Please fill in all required fields to submit your MVP features
                </p>
              )}
            </div>
...
export default MVPSubmissionComponent;