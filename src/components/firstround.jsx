...
const IdeaSubmissionComponent = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.ideaDescription.trim()) {
      errors.ideaDescription = formData.problemStatement === 'Open Innovation' 
        ? 'Please describe your idea' 
        : 'Please write about your selected idea';
    } else if (formData.ideaDescription.trim().split(/\s+/).length < 50) {
      errors.ideaDescription = 'Description must be at least 50 words';
    } else if (formData.ideaDescription.trim().split(/\s+/).length > 500) {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'problemStatement') {
      ...
      if (validationErrors.ideaDescription) {
        setValidationErrors(prev => ({
          ...prev,
          ideaDescription: ''
        }));
    ...
    try {
      ...
      if (data.success) {
        ...
        setMessageContent({
          ...
          description: 'Your idea has been submitted successfully. Our team will review it and get back to you soon. Good luck!'
        });
        setShowMessage(true);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white p-4 sm:p-8">
      ...
      <div className={`text-center mb-8 transition-all duration-1000 ${
        ...
        <h1 className="text-4xl sm:text-5xl font-bold text-white mb-4">Idea Submission</h1>
        <p className="text-green-400/80 text-lg">Submit your innovative idea for Omnitrix Hackathon 2025</p>
      </div>
                ...
                <div className="flex justify-between items-center">
                  <label className="block text-white font-semibold text-lg">
                    {formData.problemStatement === 'Open Innovation' 
                      ? 'Describe Your Idea' 
                      : 'Write About Your Selected Idea'} 
                    <span className="text-red-400"> *</span>
                  </label>
                  <span className={`text-sm font-medium ${
                    getWordCount(formData.ideaDescription) < 50 
                      ? 'text-red-400' 
                      : getWordCount(formData.ideaDescription) > 500 
                        ? 'text-red-400' 
                        : 'text-green-400'
                  }`}>
                    {getWordCount(formData.ideaDescription)} / 500 words
                  </span>
                </div>
                <textarea
                  ...
                  placeholder={formData.problemStatement === 'Open Innovation'
                    ? 'Describe your innovative idea in detail. Include the problem you\'re solving, your proposed solution, and its impact. (50-500 words)'
                    : 'Write about your approach to the selected problem statement. Include your solution concept, implementation strategy, and expected outcomes. (50-500 words)'}
                />
                {validationErrors.ideaDescription && (
                  <p className="text-red-400 text-sm mt-1">{validationErrors.ideaDescription}</p>
                )}
                <p className="text-gray-400 text-sm">
                  Minimum 50 words required. Be clear and concise about your idea.
                </p>
              </div>
            )}
            ...
            <div className="pt-6">
              ...
              {!isFormValid() && !isLoading && (
                <p className="text-yellow-400 text-sm mt-2 text-center">
                  Please fill in all required fields to submit your idea
                </p>
              )}
            </div>
...
export default IdeaSubmissionComponent;