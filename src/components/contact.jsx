import React from 'react';
import humoungImg from "../assets/way.png";
import titleImg from "../assets/foot.png";
import title from "../assets/footer.png";                                       

// Team member images
import manojImg from "../assets/manoj.jpg";
import sahithImg from "../assets/sahith.jpg";
import panduImg from "../assets/pandu.jpg";
import bharathImg from "../assets/bharath.jpg";
import pavanImg from "../assets/pavan.jpg";
import anandImg from "../assets/anand.jpg";
import ratnakarImg from "../assets/pandu.jpg";
import yagneshImg from "../assets/yagnesh.jpg";
import praveenImg from "../assets/praveen.jpg";

// Social Media Icons Components
const LinkedInIcon = () => (
  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
  </svg>
);

const InstagramIcon = () => (
  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
  </svg>
);

const WhatsAppIcon = () => (
  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.886 3.488"/>
  </svg>
);

export default function Footer() {
  // Array of team member images with names
  const teamMembers = [
    { img: manojImg, name: "Manoj" },
    { img: sahithImg, name: "Sahith" },
    { img: panduImg, name: "Pandu" },
    { img: bharathImg, name: "Bharath" },
    { img: pavanImg, name: "Pavan" },
    { img: anandImg, name: "Anand" },
    { img: ratnakarImg, name: "Ratnakar" },
    { img: yagneshImg, name: "Yagnesh" },
    { img: praveenImg, name: "Praveen" },
    // Adding one more to make it 10 members as in the original grid
    { img: humoungImg, name: "Team Lead" }
  ];

  return (
    <footer className="mt-12 px-4 sm:px-6 pb-8">
      <div className="max-w-7xl mx-auto">
        {/* Main Footer Content - Matching Registration Transparency */}
        <div className="border-2 border-green-400/30 rounded-xl p-6 sm:p-8 hover:border-green-400/60 transition-all duration-300">
          <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
            
            {/* Left Side - Alien Character */}
            <div className="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left">
              <div className="relative mb-6">
                <img 
                  src={humoungImg} 
                  alt="Alien character" 
                  className="w-32 h-32 sm:w-40 sm:h-40 lg:w-48 lg:h-48 object-contain"
                  style={{ 
                    background: 'transparent',
                    filter: 'drop-shadow(0 0 20px rgba(34, 197, 94, 0.3))'
                  }}
                />
              </div>
              
              <div className="mb-6">
                <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-2">
                  Meet at the Hackathon!
                </h3>
                <div className="text-green-400 text-lg sm:text-xl font-bold mb-4 tracking-wider">
                  OMNITRIX
                </div>
              </div>

              {/* Social Media Icons */}
              <div className="flex gap-4">
                <a 
                  href="#" 
                  className="p-3 rounded-full border-2 border-green-400/30 hover:bg-green-400/10 hover:border-green-400/60 transition-all duration-300 transform hover:scale-110 text-green-400 hover:text-green-300"
                  aria-label="LinkedIn"
                >
                  <LinkedInIcon />
                </a>
                <a 
                  href="#" 
                  className="p-3 rounded-full border-2 border-green-400/30 hover:bg-green-400/10 hover:border-green-400/60 transition-all duration-300 transform hover:scale-110 text-green-400 hover:text-green-300"
                  aria-label="Instagram"
                >
                  <InstagramIcon />
                </a>
                <a 
                  href="#" 
                  className="p-3 rounded-full border-2 border-green-400/30 hover:bg-green-400/10 hover:border-green-400/60 transition-all duration-300 transform hover:scale-110 text-green-400 hover:text-green-300"
                  aria-label="WhatsApp"
                >
                  <WhatsAppIcon />
                </a>
              </div>
            </div>

            {/* Middle - Team Section */}
            <div className="flex-1 flex flex-col items-center">
              <div className="text-center mb-6">
                <h3 className="text-lg sm:text-xl font-bold text-white mb-2 tracking-wide">OUR ALIENS</h3>
                <div className="w-12 h-0.5 bg-gradient-to-r from-green-400 to-emerald-500 mx-auto rounded-full shadow-lg shadow-green-400/50"></div>
              </div>
              
              {/* Frame around team section */}
              <div className="border border-green-400/30 rounded-lg p-4 bg-transparent">
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 sm:gap-4 max-w-sm">
                  {teamMembers.map((member, index) => (
                    <div key={index} className="flex justify-center">
                      <a 
                        href="#" 
                        className="block"
                        aria-label={`Team member ${member.name}`}
                      >
                        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full border-2 border-green-400/40 overflow-hidden shadow-md shadow-green-400/20">
                          <img 
                            src={member.img} 
                            alt={`Team member ${member.name}`} 
                            className="w-full h-full object-cover"
                            style={{ 
                              background: 'transparent',
                              filter: 'drop-shadow(0 0 8px rgba(34, 197, 94, 0.2))'
                            }}
                          />
                        </div>
                      </a>
                    </div>
                  ))}
                </div>
              </div>
            </div>

            {/* Right Side - Title Image */}
            <div className="flex-1 flex justify-center lg:justify-end">
              <div className="relative">
                <img 
                  src={titleImg} 
                  alt="Title image"
                  className="max-w-full h-auto object-contain w-64 sm:w-80 lg:w-96"
                  style={{ 
                    background: 'transparent',
                    filter: 'brightness(1.2) contrast(1.1) drop-shadow(0 0 15px rgba(34, 197, 94, 0.2))'
                  }}
                />
              </div>
            </div>
          </div>
        </div>

        {/* Footer Section */}
        <section className="mt-12">
          <div className="w-full max-w-none mx-auto px-0">
            {/* Footer Image */}
            <div className="relative w-full h-80 sm:h-96 lg:h-[500px] xl:h-[600px] rounded-2xl overflow-hidden border-2 border-green-400/30 shadow-2xl shadow-green-400/20 mb-8">
              <img 
                src={title} 
                alt="Footer" 
                className="w-full h-full object-cover object-center"
                style={{ 
                  objectFit: 'cover',
                  objectPosition: 'center'
                }}
              />
              {/* Overlay gradient for better text readability if needed */}
              <div className="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-transparent"></div>
            </div>
          </div>
        </section>

        {/* Bottom Footer - Copyright */}
        <div className="mt-6 pt-4 border-t border-green-400/20">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-gray-400">
            <div className="flex items-center gap-6">
              <span>© 2025 OMNITRIX. All rights reserved.</span>
              <span className="hidden sm:inline">|</span>
              <span className="text-green-400">Hackathon Innovation Platform</span>
            </div>
            <div className="flex items-center gap-4 text-xs">
              <a href="#" className="hover:text-green-400 transition-colors">Privacy Policy</a>
              <span>|</span>
              <a href="#" className="hover:text-green-400 transition-colors">Terms of Service</a>
              <span>|</span>
              <span className="text-green-400">Made with 💚 for Innovation</span>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}